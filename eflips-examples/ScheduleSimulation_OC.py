# -*- coding: utf-8 -*-
"""Demo script for bus ScheduleSimulation with opportunity charging (OC).
...
schedule_simulation_params = {
    ...
    'charging_point_params': {
        ...
        # End-stop charging points will be added later (we need to find
        # their IDs first).
    },
    'depot_params': {
        'charging': True,
...
# Update opportunity charging station locations:
for name in charging_point_location_names:
    grid_point = grid.find_points('name', name)[0]
    schedule_simulation_params['charging_point_params'].update({
        grid_point.ID: {
            'interface': 'pantograph_300',
            'capacity': 2
        }
    })
...
for cp_id, cp_data in \
        simulation.simulation_info['logger_data']['charging_points'].items():
    eflips.evaluation.plot_retriever_data(
